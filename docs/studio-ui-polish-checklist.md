...
- [~] **17. Replace repeated multi-file downloads with a scalable ZIP/download-batch flow.** Implementation in validation: Gallery bulk Download now makes one POST to `/api/download-batch`, which builds a single ZIP from persisted R2 originals (up to 100 items / 1 GB) and triggers one browser download instead of many independent downloads.
