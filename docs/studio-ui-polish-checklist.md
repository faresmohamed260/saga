...
- [~] **16. Harden bulk destructive failure handling.** Implementation in validation: bulk delete now records per-item outcomes, removes successful deletions immediately, reports partial failure counts/details, and leaves only failed items selected for retry.
