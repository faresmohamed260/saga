...
- [~] **18. Replace CreateWorkspace portal/querySelector integration with explicit component slots/composition.** Implementation in validation: legacy composer now exposes explicit `videoToolbarSlot` and `composerStatusSlot` React slots; the feature wrapper passes VideoOutputControls and VideoGenerationProgress directly, with `createPortal` and CSS-class `document.querySelector` host discovery removed.
