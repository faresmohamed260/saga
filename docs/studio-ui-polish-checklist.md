...
- [~] **23. Add Gallery search and sorting.** Implementation in validation: prompt search is server-backed across completed generations, Newest/Oldest sorting is explicit and stable, filters remain composable with kind/model pagination, and responsive controls are covered by the Gallery visual harness.
