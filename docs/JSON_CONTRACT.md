...
The legacy JSON contract is no longer the primary production storage layer.

The current production path stores analysis state in SQLite first. JSON exports remain a compatibility or export surface for inspection, download, and interchange when needed.
...
`scene_analyses` and `resolved_scene_analyses` are still important in exports because some downstream rebuilders and audits depend on them.
...
With the current production identity path, exports should preserve provider-backed identity data such as:
...
Recent hardening work changed export expectations for failed or degraded runs.

A catastrophic scene-failure run should no longer look like a healthy successful export with empty canon artifacts.
...
- the export intentionally excludes live Python service objects
- the export is meant to be deterministic enough for downstream rebuild and audit workflows
- export validation is available through `saga_tools.py validate-encoder-artifacts`
