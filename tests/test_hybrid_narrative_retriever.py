...
def test_hybrid_embedding_index_service_persists_and_queries_scoped_results(tmp_path):
    service = HybridEmbeddingIndexService(base_dir=tmp_path, embedder=_fake_embedder)
    docs = _retrieval_context()["retrieval_documents"]

    payload = service.ensure_index(series_id="acotar", scope_key="acosf", documents=docs)
    again = service.ensure_index(series_id="acotar", scope_key="acosf", documents=docs)
...
def test_hybrid_retriever_builds_monologue_scene_packet_with_narrow_focus(tmp_path):
    retriever = HybridNarrativeRetriever(
        index_service=HybridEmbeddingIndexService(base_dir=tmp_path, embedder=_fake_embedder)
    )
