...
from packages.execution_runtime.service import (
...
class FakeExecutor:
    def __init__(
        self, request_cancelled, *, status="accepted", error=None, on_run=None
    ) -> None:
    ...
    def run(self, request, *, thread_id=""):
        ...
        return OrchestrationResult(
            ...
            decision=OrchestrationDecisionArtifact(
                ...
                accepted=accepted,
                status=status,
                failed_stage=None if accepted else "artifact_packaging",
                completed_stages=["artifact_packaging"] if accepted else [],
...
def _persistence(tmp_path: Path):
    profile = PersistenceProfile(
        name="execution-runtime-test",
        provider="supabase",
        mode="test_harness",
        database_url=f"sqlite:///{tmp_path / 'execution.sqlite3'}",
        local_storage_root_dir=str(tmp_path / "storage"),
    )
    client = create_persistence_client(
        profile=profile, config=PersistenceRuntimeConfig(profile=profile)
    )
    client.initialize()
    return client
...
def test_retry_resets_only_top_level_orchestration_checkpoint(monkeypatch):
    request = _request("run-retry")
    queue_id = "execution-" + hashlib.sha256(b"run-retry").hexdigest()[:24]

    class QueueStub:
        def get(self, value):
            assert value == queue_id
    ...
    deleted = []
    monkeypatch.setattr(
        "packages.execution_runtime.service.SqlCheckpointSaver",
        lambda **_: SimpleNamespace(delete_thread=deleted.append),
    )
    service = object.__new__(ExecutionRuntimeService)
...
def test_trace_snapshot_loading_deduplicates_and_enforces_aggregate_bound():
    payloads = {
        ("reports", "one.json"): json.dumps({"value": "one"}),
        ("reports", "two.json"): json.dumps({"value": "two"}),
    }
    ...
    class ObjectsStub:
        ...
        def download_text(self, bucket, path):
            self.calls.append((bucket, path))
            return payloads[(bucket, path)]
    ...
    records = [
        ...
        },
        {
            "payload": {
                "output_artifact_version": {
                    "bucket_name": "reports",
                    "object_path": "two.json",
                }
            }
        },
    ]
    snapshots = load_lineage_trace_snapshots(
        SimpleNamespace(objects=objects),
        records,
        max_characters=100,
        max_total_characters=len(payloads[("reports", "one.json")]),
    )

    assert snapshots == [{"value": "one"}]
    assert objects.calls == [("reports", "one.json"), ("reports", "two.json")]
...
def test_admission_enforces_series_and_capability_limits(tmp_path: Path):
    ...
    store.set_policy(
        "q",
        {
            "global_limit": 3,
            "per_series_limit": 1,
            "default_capability_limit": 3,
            "capability_limits": {"reasoning": 2},
        },
    )
    store.enqueue(
        "q-a",
...
def test_concurrent_workers_cannot_claim_same_capacity_slot(tmp_path: Path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(
            pool.map(lambda worker: store.claim("q", worker_id=worker), ["w1", "w2"])
        )
    assert len([item for item in results if item]) == 1


def test_expired_worker_cannot_heartbeat_or_complete_and_work_is_recovered(
    tmp_path: Path,
):
    client = _persistence(tmp_path)
    store = client.execution_queue
    ...
    assert (
        store.heartbeat(
            "q-a", worker_id="old", lease_token=item["lease_token"], now_ms=expired
        )
        is None
    )
    assert (
...
def test_cancellation_prevents_queued_claim_and_marks_leased_work(tmp_path: Path):
    ...
    assert (
        store.request_cancel("queued", reason="user request")["status"] == "cancelled"
    )
    ...
    assert (
        store.complete("leased", worker_id="w1", lease_token=item["lease_token"])[
            "status"
        ]
        == "cancelled"
    )
...
def test_terminal_observer_receives_result_and_failure_cannot_reverse_success(
    tmp_path: Path,
):
    client = _persistence(tmp_path)
    queue = ExecutionQueueRuntime(persistence=client)
...
def test_worker_propagates_cancellation_and_nonretryable_error(tmp_path: Path):
    ...
    cancelled = ExecutionWorker(
        ...
        enable_heartbeat=False,
        executor_factory=lambda checker: FakeExecutor(
            checker, on_run=lambda: queue.cancel(item["queue_id"])
        ),
    ).run_once()
    ...
    failed = ExecutionWorker(
        ...
        enable_heartbeat=False,
        executor_factory=lambda checker: FakeExecutor(
            checker, error=ValueError("invalid request")
        ),
    ).run_once()
...
def test_terminal_purge_is_scoped_and_rejects_active_work(tmp_path: Path):
    ...
    purged = store.purge_terminal(
        "qualification", run_ids=["terminal-run", "active-run"]
    )
