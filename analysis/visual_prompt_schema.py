...
def compile_location_concept_prompt(
    *,
    ...
    lines = [
        f"Create a photorealistic environmental concept image of {name}.",
    ]
    if baseline:
        lines.append(_sentence(f"Baseline physical setting: {baseline}"))
    if current and current.lower() != baseline.lower():
        lines.append(_sentence(f"Current visual state: {current}"))
    if mood:
        lines.append(_sentence(f"Atmosphere: {mood}"))
    if features:
        lines.append(_sentence(f"Notable visible features: {', '.join(features[:5])}"))
    if damage:
        lines.append(_sentence(f"Damage or restoration state: {damage}"))
    lines.extend(
        [
            "Keep the architecture, terrain, materials, lighting, and environmental storytelling grounded in the source setting.",
            "Photorealistic rendering, strong spatial coherence, natural materials, and no stylized illustration look.",
        ]
    )
    return "\n".join(line for line in lines if line).strip()
