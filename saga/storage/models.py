...
class AudiobookRun(Base, TimestampMixin):
    ...
    job_id: Mapped[str | None] = mapped_column(ForeignKey("dashboard_jobs.id"), nullable=True, index=True)
    ...
    tts_provider: Mapped[str | None] = mapped_column(String(128), nullable=True)
    tts_app_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    provider_account_alias: Mapped[str | None] = mapped_column(String(255), nullable=True)
    voice: Mapped[str | None] = mapped_column(String(128), nullable=True)
    lang_code: Mapped[str | None] = mapped_column(String(32), nullable=True)
    sample_rate: Mapped[int | None] = mapped_column(Integer, nullable=True)
    audio_format: Mapped[str | None] = mapped_column(String(32), nullable=True)
    normalize_audio: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    trim_silence: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    sentence_pause_ms: Mapped[int | None] = mapped_column(Integer, nullable=True)
    total_books: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    failed_chapters: Mapped[int | None] = mapped_column(Integer, nullable=True)
    transcript_storage_mode: Mapped[str | None] = mapped_column(String(32), nullable=True)
    audio_storage_mode: Mapped[str | None] = mapped_column(String(32), nullable=True)
    progress_json: Mapped[dict | list | None] = mapped_column(JSON, nullable=True)
    error: Mapped[str | None] = mapped_column(Text, nullable=True)
    metadata_json: Mapped[dict | list | None] = mapped_column(JSON, nullable=True)


class AudiobookChapter(Base, TimestampMixin):
    __tablename__ = "audiobook_chapters"
    __table_args__ = (UniqueConstraint("run_id", "chapter_id", name="uq_audiobook_chapters_run_chapter"),)

    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=_uuid)
    run_id: Mapped[str] = mapped_column(ForeignKey("audiobook_runs.id"), index=True)
    series_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    book_id: Mapped[str] = mapped_column(ForeignKey("books.id"), index=True)
    ...
    chapter_index: Mapped[int | None] = mapped_column(Integer, nullable=True)
    chapter_title: Mapped[str | None] = mapped_column(Text, nullable=True)
    transcript_status: Mapped[str | None] = mapped_column(String(32), nullable=True, index=True)
    audio_status: Mapped[str | None] = mapped_column(String(32), nullable=True, index=True)
    ...
    audio_byte_size: Mapped[int | None] = mapped_column(Integer, nullable=True)
