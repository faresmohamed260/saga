...
const runtimeState = {
  ...
  prompts: [
    {
      path: "analysis/scene_analyzer.py",
      name: "scene_analyzer.py",
      line_count: 10,
      prompt_hits: ["You are a scene analyst."],
      content: "You are a scene analyst.",
    },
  ],
};
...
test("renders project-owned local dashboard without workspace picker", async () => {
  render(<App />);
  expect(await screen.findByText("S.A.G.A. Local Operations Dashboard")).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "New Encode Run" })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Prompt Inspector" })).toBeInTheDocument();
