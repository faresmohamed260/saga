...
from packages.generation_planning import GenerationPlanningService, load_generation_planning_service_config_from_env
...
def main() -> int:
    ...
    if args.resume:
        os.environ["SAGA_CANON_RESUME_STAGES"] = "event_extraction,entity_extraction,relationship_extraction"
    ...
    try:
        _freshness_guard(service, source=source, source_sha256=source_sha256, series_id=series_id, allow_resume=args.resume)
        ...
        queued = service.submit(request, max_attempts=2, backoff_seconds=0)
        if args.resume and queued and queued.get("status") in {"cancelled", "dead_letter"}:
            queued = service.retry(request, max_attempts=2)
        if queued is None:
            raise RuntimeError("Qualification queue submission did not return an item.")
        queue_id = str(queued["queue_id"])
        _emit("submitted", run_id=run_id, series_id=series_id, queue_id=queue_id, source_sha256=source_sha256)
        outcome: dict[str, object] = {}

        def work() -> None:
            try:
                outcome["result"] = service.run_worker_once(worker_id=f"qualification-worker-{suffix}")
            except BaseException as exc:  # noqa: BLE001
                outcome["error"] = {"type": type(exc).__name__, "message": str(exc)[:1000]}

        worker = threading.Thread(target=work, name=f"qualification-{suffix}", daemon=True)
        worker.start()
        ...
        while worker.is_alive() and time.monotonic() < deadline:
            ...
            for log in job.get("logs") or []:
                ...
                _emit("stage_event", stage=stage, message=log.get("message"), elapsed_seconds=round(time.monotonic() - started, 1))
            if current_stage and time.monotonic() - stage_started > max(60, args.stage_timeout_seconds):
                cancellation_reason = f"Stage deadline exceeded: {current_stage}"
                service.cancel(queue_id, reason=cancellation_reason)
                _emit("stage_deadline", stage=current_stage, elapsed_seconds=round(time.monotonic() - stage_started, 1))
                break
        ...
        if worker_result.orchestration_result is None:
            _emit("worker_terminal", status=worker_result.status, error=worker_result.error)
            return 2
        if not worker_result.orchestration_result.decision.accepted:
            ...
            return 3 if worker_result.orchestration_result.decision.status == "cancelled" else 2
        ...
        failed = [item.model_dump() for item in report.checks if item.status != "passed"]
        _emit(
            "qualification_complete", accepted=report.accepted, run_id=run_id, series_id=series_id,
            elapsed_seconds=round(time.monotonic() - started, 1), failed_checks=failed,
            metrics=report.metrics, report_artifact=report.artifact_reference,
        )
...
def _freshness_guard(service: ExecutionRuntimeService, *, source: Path, source_sha256: str, series_id: str, allow_resume: bool) -> None:
    existing_series = next((item for item in service.persistence.library.list_series(limit=10000) if item.get("series_id") == series_id), None)
    existing_books = service.persistence.library.list_books(limit=10000)
    source_name = source.name.casefold()
    matching_sources = [item for item in existing_books if source_name in str(item.get("source_uri") or "").casefold() or source_sha256 in json.dumps(item, sort_keys=True)]
    if not allow_resume and (existing_series is not None or matching_sources):
        raise RuntimeError(f"Freshness guard rejected existing input/state: series={existing_series is not None}, source_matches={len(matching_sources)}")
...
def _reasoning_preflight(*, timeout_seconds: int) -> None:
    ...
    try:
        ...
        if "QUALIFICATION_READY" not in str(response or "").upper():
            raise RuntimeError("Reasoning provider preflight returned an unexpected response.")
        metadata = service.runtime.reasoning_runtime.last_request_metadata()
        _emit("reasoning_preflight", provider=service.runtime.reasoning_runtime.provider_name(), model=service.runtime.reasoning_runtime.resolved_model_name(), status=metadata.get("status") if isinstance(metadata, dict) else getattr(metadata, "status", ""))
    finally:
        service.persistence.close()
