...
def test_build_decoder_uses_hybrid_reasoning_bundle_when_general_compute_is_ready(monkeypatch, tmp_path):
    store = SagaSQLiteStore(tmp_path / "saga.sqlite")
    service = DatabaseDecoderService(sqlite_store=store)

    monkeypatch.setattr(service, "_provider_ready", lambda provider_name: provider_name == "general_compute")

    decoder = service._build_decoder()

    assert decoder.llm.mode == "gpt_oss"
    assert decoder.prose_llm.mode == "gpt_oss"
    assert decoder.planner_llm.mode == "general_compute"
