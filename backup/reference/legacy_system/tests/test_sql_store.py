...
def test_sql_store_persists_contract_and_render_manifest(tmp_path: Path) -> None:
    ...
    with store.session_factory() as session:
        ...
        assert len(images) == 1
        assert images[0].output_path == str(image_path)
        assert images[0].image_bytes is None
...
def test_sql_store_persists_render_manifest_for_db_book_ref(tmp_path: Path) -> None:
    ...
    with store.session_factory() as session:
        image = session.execute(select(GeneratedImage)).scalar_one()
        assert image.output_path == str(image_path)
        assert image.thumbnail_path is None


def test_sql_store_persists_existing_thumbnail_metadata_without_loading_image_bytes(tmp_path: Path) -> None:
    db_path = tmp_path / "saga.sqlite3"
    store = SagaSQLiteStore(db_path)
    contract_path = tmp_path / "contract.json"
    store.persist_contract(_sample_contract(tmp_path), contract_path=contract_path)

    image_path = tmp_path / "feyre-thumb.png"
    image_path.write_bytes(b"fakepngthumb")
    thumb_path = thumbnail_path_for(image_path)
    thumb_path.write_bytes(b"jpegthumb")
    ...
    with store.session_factory() as session:
        ...
        assert image.thumbnail_path == str(thumb_path)
