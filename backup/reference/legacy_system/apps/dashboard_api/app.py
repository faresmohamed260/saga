...
from saga.storage.render_artifact_projection import (
...
DEFAULT_TTS_MODAL_APP_NAME = "saga-tts-runtime"
...
SQLITE_STORE = SagaRelationalStore()
...
def _safe_probe_ollama_account(account: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        probe = probe_runtime_mode_access(MODE_GPT_OSS, timeout=30, max_retries=1)
    except Exception as exc:
        probe = {"status": "error", "detail": repr(exc)}
    model_name = str(probe.get("model") or create_runtime_client(mode=MODE_GPT_OSS).resolved_model_name())
...
def _safe_probe_general_compute_account(account: dict[str, Any]) -> dict[str, Any]:
    raw = _read_provider_config(GENERAL_COMPUTE_PROVIDER, mask=False)
    ...
    api_key = str(account.get("api_key") or "").strip() or None
    try:
        probe = probe_runtime_mode_access(MODE_GENERAL_COMPUTE, timeout=30, max_retries=1)
    except Exception as exc:
        probe = {"status": "error", "detail": repr(exc)}
    model_name = str(probe.get("model") or create_runtime_client(mode=MODE_GENERAL_COMPUTE).resolved_model_name())
    limits = dict(raw_match.get("limits") or {}) if isinstance(raw_match, dict) and isinstance(raw_match.get("limits"), dict) else {}
    usage = dict(raw_match.get("usage") or {}) if isinstance(raw_match, dict) and isinstance(raw_match.get("usage"), dict) else {}
    return {
...
def _safe_probe_codex_account(account: dict[str, Any]) -> dict[str, Any]:
    api_key = str(account.get("api_key") or "").strip() or None
    try:
        probe = probe_runtime_mode_access(MODE_CODEX, timeout=30, max_retries=1)
    except Exception as exc:
        probe = {"status": "error", "detail": repr(exc)}
    model_name = str(probe.get("model") or create_runtime_client(mode=MODE_CODEX).resolved_model_name())
...
def _safe_probe_inference_provider(provider_name: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if provider_key == MODAL_KOKORO_PROVIDER:
        ...
        resolved_model = str(config.get("app_name") or "saga-image-runtime")
    return {
    ...
    with SQLITE_STORE.session_factory() as session:
        ...
        images = session.execute(
            select(
                ...
                SqlGeneratedImage.thumbnail_path,
                SqlGeneratedImage.render_status,
                SqlGeneratedImage.workflow_name,
        ...
        for entity in entities:
            ...
            entity_render_artifact = _render_artifact_payload(
                str(entity.generated_image_path or str((image or {}).get("output_path") or "")),
                thumbnail_path=str(entity.generated_thumbnail_path or str((image or {}).get("thumbnail_path") or "")),
                render_status=str((image or {}).get("render_status") or ""),
                workflow_name=str((image or {}).get("workflow_name") or ""),
            )
            entity_row["render_artifact"] = entity_render_artifact
            ...
            visual_inventory.append({
                **entity_row,
                ...
                "change_prompts": [],
                "scene_prompts": [],
                "generated_image_path": entity_render_artifact["output_path"],
                "generated_thumbnail_path": entity_render_artifact["thumbnail_path"],
                "negative_prompt": str(getattr(prompt, "negative_prompt", "") or ""),
                "render_status": entity_render_artifact["render_status"],
                "workflow_name": entity_render_artifact["workflow_name"],
                "render_artifact": entity_render_artifact,
            })
        ...
        return {
            ...
            "render_summary": {
                ...
                "rendered_count": len([row for row in images if str(row.output_path or "").strip() and Path(str(row.output_path or "")).exists()]),
            },
            "truncated_at": limit,
        }
...
def _ensure_runtime_thumbnail(image_path: str, thumbnail_path: str = "") -> str:
    return resolve_runtime_thumbnail(image_path, thumbnail_path)
...
def _entity_render_artifact(
    *,
    entity_image_path: str = "",
    entity_thumbnail_path: str = "",
    latest_image: dict[str, Any] | None = None,
) -> dict[str, Any]:
    return build_entity_render_artifact_projection(
        entity_image_path=entity_image_path,
        entity_thumbnail_path=entity_thumbnail_path,
...
def run_audiobook_job(job_id: str, run_id: str) -> None:
    ...
    rewrite_llm = create_runtime_client(mode=_audiobook_llm_mode(rewrite_provider), allow_cross_provider_fallback=False)
...
def runtime_assets_entities(entity_type: str = "", q: str = "", book_id: str = "", series_id: str = "", limit: int = 48, offset: int = 0) -> dict[str, Any]:
    with SQLITE_STORE.session_factory() as session:
        ...
        if entity_ids:
            ...
            image_rows = session.execute(
                select(
                    ...
                    SqlGeneratedImage.workflow_name.label("workflow_name"),
                )
                .where(SqlGeneratedImage.entity_id.in_(entity_ids))
        ...
        for row in rows:
            ...
            render_artifact = _entity_render_artifact(
                entity_image_path=str(row.get("generated_image_path") or "").strip(),
                entity_thumbnail_path=str(row.get("generated_thumbnail_path") or "").strip(),
                latest_image=image,
            )
            output.append(
                {
                    ...
                    "render_status": render_artifact["render_status"],
                    "generated_image_path": render_artifact["output_path"],
                    "generated_thumbnail_path": render_artifact["thumbnail_path"],
...
def runtime_asset_entity(entity_id: str) -> dict[str, Any]:
    with SQLITE_STORE.session_factory() as session:
        ...
        entity_render_artifact = _entity_render_artifact(
            entity_image_path=entity.generated_image_path or "",
            entity_thumbnail_path=entity.generated_thumbnail_path or "",
            latest_image={
                "output_path": latest_image.output_path if latest_image else "",
                "thumbnail_path": latest_image.thumbnail_path if latest_image else "",
                "render_status": latest_image.render_status if latest_image else "",
                "workflow_name": latest_image.workflow_name if latest_image else "",
            },
        )
        entity_payload = {
            "id": entity.id,
        ...
        image_rows = []
        for row in images:
            artifact = _render_artifact_payload(
                row.output_path or "",
...
def runtime_mark_preferred_image(image_id: str) -> dict[str, Any]:
    with SQLITE_STORE.session_factory() as session:
        ...
        entity.generated_thumbnail_path = _ensure_runtime_thumbnail(image.output_path or "", image.thumbnail_path or "")
        session.commit()
