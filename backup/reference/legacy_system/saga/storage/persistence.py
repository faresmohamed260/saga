...
from saga.services.image_thumbnail_service import thumbnail_path_for


class _SagaRelationalStoreBase:
    """Canonical relational persistence for encoder and visual outputs."""
    ...
    def __init__(self, database_path: str | Path | None = None, *, database_url: str | None = None) -> None:
        ...
        self.session_factory = get_session_factory(database_path, database_url)


class SagaRelationalStore(_SagaRelationalStoreBase):
    """Canonical relational persistence implementation."""
    ...
    def persist_render_manifest(self, manifest: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.session_factory() as session:
            ...
            for row in (manifest.get("renders") or []):
                ...
                thumbnail_path = self._existing_thumbnail_path(
                    str(row.get("thumbnail_path") or "").strip() or output_path
                )
                should_store_image = bool(output_path)
    ...
    @staticmethod
    def _existing_thumbnail_path(source_path: str) -> str:
        source = str(source_path or "").strip()
        if not source:
            return ""
        candidate = Path(source)
        if candidate.suffix.lower() in {".jpg", ".jpeg"} and candidate.exists():
            return str(candidate)
        derived = thumbnail_path_for(candidate)
    ...
    def _provider_config_dict(self, session: Session, row: ProviderConfig) -> dict[str, Any]:
        ...
        for idx, account in enumerate(normalized_accounts):
            ...
            account["index"] = idx
            account["metadata"] = {
                **(dict(account.get("metadata") or {}) if isinstance(account.get("metadata"), dict) else {}),
                "label": account["label"],
                "index": idx,
...
class SagaSQLiteStore(SagaRelationalStore):
    """Backward-compatible alias for the default SQLite-backed relational store."""
