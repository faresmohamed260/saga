from __future__ import annotations

import os
from pathlib import Path

from sqlalchemy import create_engine, text
from sqlalchemy.orm import sessionmaker

from .models import Base


PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_DB_PATH = PROJECT_ROOT / "analysis_outputs" / "saga_canonical.sqlite3"
DEFAULT_DATABASE_URL = f"sqlite:///{DEFAULT_DB_PATH}"

_ENGINES: dict[str, object] = {}
_SESSION_FACTORIES: dict[str, object] = {}


def get_database_url(database_path: str | Path | None = None, database_url: str | None = None) -> str:
    explicit_url = str(database_url or os.getenv("SAGA_DATABASE_URL") or "").strip()
    if explicit_url:
        return explicit_url
    explicit_path = str(database_path or os.getenv("SAGA_SQLITE_PATH") or "").strip()
    target = Path(explicit_path) if explicit_path else DEFAULT_DB_PATH
    if not target.is_absolute():
        target = (PROJECT_ROOT / target).resolve()
    target.parent.mkdir(parents=True, exist_ok=True)
    return f"sqlite:///{target}"


def get_engine(database_path: str | Path | None = None, database_url: str | None = None):
    resolved_url = get_database_url(database_path, database_url)
    engine = _ENGINES.get(resolved_url)
    if engine is None:
        engine = create_engine(
            resolved_url,
            future=True,
            echo=False,
        )
        _ENGINES[resolved_url] = engine
    return engine


def get_session_factory(database_path: str | Path | None = None, database_url: str | None = None):
    resolved_url = get_database_url(database_path, database_url)
    factory = _SESSION_FACTORIES.get(resolved_url)
    if factory is None:
        factory = sessionmaker(
            bind=get_engine(database_path, database_url),
            autoflush=False,
            expire_on_commit=False,
            future=True,
        )
        _SESSION_FACTORIES[resolved_url] = factory
    return factory


def initialize_database(database_path: str | Path | None = None, database_url: str | None = None) -> None:
    engine = get_engine(database_path, database_url)
    Base.metadata.create_all(engine)
    if str(engine.url).startswith("sqlite:"):
        _ensure_sqlite_migrations(engine)


def _ensure_sqlite_migrations(engine) -> None:
    with engine.begin() as connection:
        columns = {row[1] for row in connection.execute(text("PRAGMA table_info(entities)")).fetchall()}
        if "baseline_visual_prompt" not in columns:
            connection.execute(text("ALTER TABLE entities ADD COLUMN baseline_visual_prompt TEXT"))
        if "generated_image_path" not in columns:
            connection.execute(text("ALTER TABLE entities ADD COLUMN generated_image_path TEXT"))
        if "generated_thumbnail_path" not in columns:
            connection.execute(text("ALTER TABLE entities ADD COLUMN generated_thumbnail_path TEXT"))
        if "generated_image_bytes" not in columns:
            connection.execute(text("ALTER TABLE entities ADD COLUMN generated_image_bytes BLOB"))
        image_columns = {row[1] for row in connection.execute(text("PRAGMA table_info(generated_images)")).fetchall()}
        if "thumbnail_path" not in image_columns:
            connection.execute(text("ALTER TABLE generated_images ADD COLUMN thumbnail_path TEXT"))
