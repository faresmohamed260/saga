...
from saga.retrieval.target_character_state_service import TargetCharacterStateService, _target_scope_label
from saga.identity.identity_provider import resolve_identity_provider_input
from saga.services.relational_contract_adapter import is_db_book_ref, load_contract_like
...
class VisualWorldStateService:
    ...
    def _build_entity_visual_states(
        ...
        *,
        ...
        rows: List[Dict[str, Any]] = []
        noise: List[Dict[str, Any]] = []
        entity_items = list(entity_registry or [])
        seen_entity_keys = {
            (_norm(str(item.get("name") or "")), str(item.get("entity_type") or "").strip().lower())
            for item in entity_items
        }
        for scene in filtered_scenes:
            for desc in scene.get("entity_descriptions") or []:
                entity_type = str(desc.get("entity_type") or "").strip().lower()
                display_name = str(desc.get("entity_name") or desc.get("name") or "").strip()
                key = (_norm(display_name), entity_type)
                if display_name and entity_type and entity_type not in {"character", "location"} and key not in seen_entity_keys:
                    entity_items.append({"name": display_name, "entity_type": entity_type, "descriptions": [desc], "state_changes": []})
                    seen_entity_keys.add(key)
            for change in scene.get("state_changes") or []:
                entity_type = str(change.get("entity_type") or "").strip().lower()
                display_name = str(change.get("entity_name") or change.get("name") or "").strip()
                key = (_norm(display_name), entity_type)
                if display_name and entity_type and entity_type not in {"character", "location"} and key not in seen_entity_keys:
                    entity_items.append({"name": display_name, "entity_type": entity_type, "descriptions": [], "state_changes": [change]})
                    seen_entity_keys.add(key)
