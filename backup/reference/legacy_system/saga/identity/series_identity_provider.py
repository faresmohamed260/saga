...
def _clean_surface(text: str) -> str:
    ...
    fixed = fixed.strip("\"'أ¢â‚¬إ“أ¢â‚¬â€Œأ¢â‚¬ع©أ¢â‚¬â„¢.,;:!?")
    fixed = re.sub(r"\s+[أ¢â‚¬â€‌-]\s+", "-", fixed)
    return fixed
...
def _risk_flags_for_booknlp_character(
    ...
    if any("أ¯طںآ½" in p or "ط¤ظ¾" in p or "ط£آ¢" in p for p in proper + common):
...
def generate_book_identity_bundle(
    *,
    ...
    processor = SeriesProcessor(
        llm_client=create_runtime_client(mode=MODE_GPT_OSS, max_retries=1, base_delay=0.0, timeout=60)
    )
