...
from saga.persistence.identity_store import IdentityStore
from saga.persistence.job_store import JobStore
from saga.persistence.library_store import LibraryStore
from saga.providers.reasoning_runtime_adapter import MODE_GPT_OSS, build_analysis_reasoning_client
...
class DatabaseAnalysisRunService:
    ...
    def __init__(
        ...
        *,
        ...
        self.library_store = library_store or LibraryStore(self.relational_store)
        self.identity_store = identity_store or IdentityStore(self.relational_store)
        self.job_store = job_store or JobStore(self.relational_store)
    ...
    def validate_import_plan_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        with self.relational_store.session_factory() as session:
            for item in books:
    ...
    def _load_resume_snapshot(
        ...
        *,
        ...
        if not retry_of:
            return None
        previous_job = self.job_store.get_dashboard_job(retry_of)
        if previous_job is None:
            return None
        series_id = str(request_payload.get("series_id") or "").strip()
        identity_provider = str(shared_config.get("identity_provider") or "booknlp_clean").strip().lower()
        identity_payload = self.identity_store.get_identity_series_payload(series_id) if series_id and identity_provider == "booknlp_clean" else None
    ...
    def _completed_stage_names_for_book(self, *, job_id: str, book_id: str) -> set[str]:
        ...
        for line in self.job_store.get_dashboard_job_log_tail(job_id, limit=5000):
    ...
    def _run_one_stage(
        ...
        *,
        ...
        chapter_indices: list[int],
        shared_config: dict[str, Any],
        llm_client: ReasoningClient | None,
        job_id: str,
        book_result: dict[str, Any],
        completed_units: int,
        total_units: int,
        ...
        if stage == "character_profiles":
            return DatabaseCharacterProfileAgent(llm_client=llm_client, sqlite_store=self.relational_store).analyze_book(
                book_ref=book_ref,
                limit_characters=self._optional_int(shared_config.get("limit_characters")),
            )
        if stage == "character_visual_baselines":
            return DatabaseCharacterVisualBaselineAgent(llm_client=llm_client, sqlite_store=self.relational_store).analyze_book(
                book_ref=book_ref,
                limit_characters=self._optional_int(shared_config.get("limit_characters")),
            )
        if stage == "noncharacter_visual_baselines":
            ...
            return DatabaseNonCharacterVisualBaselineAgent(llm_client=llm_client, sqlite_store=self.relational_store).analyze_book(
                book_ref=book_ref,
        ...
        if stage == "character_visual_scene_states":
            return DatabaseCharacterVisualSceneStateAgent(llm_client=llm_client, sqlite_store=self.relational_store).analyze_book(
                book_ref=book_ref,
                max_scenes=self._optional_int(shared_config.get("limit_scene_states")),
            )
        if stage == "noncharacter_scene_states":
            return DatabaseNonCharacterSceneStateAgent(sqlite_store=self.relational_store).analyze_book(book_ref=book_ref)
        if stage == "relationships":
            return DatabaseRelationshipAgent(sqlite_store=self.relational_store).analyze_book(book_ref=book_ref)
        if stage == "timeline":
            return DatabaseTimelineAgent(sqlite_store=self.relational_store).analyze_book(book_ref=book_ref)
        if stage == "stable_states":
            return DatabaseStableCharacterStateAgent(sqlite_store=self.relational_store).analyze_book(book_ref=book_ref)
        if stage == "world_state":
            return DatabaseWorldStateConsolidationAgent(sqlite_store=self.relational_store).analyze_book(book_ref=book_ref)
        if stage == "visual_prompts":
            result = EntityVisualPromptService(self.relational_store).build_book_prompts(book_ref, overwrite=True)
            return {
    ...
    def _llm_client(self, shared_config: dict[str, Any]) -> ReasoningClient:
        model_mode = str(shared_config.get("analysis_model") or MODE_GPT_OSS).strip() or MODE_GPT_OSS
        resolved_config = dict(shared_config)
        resolved_config["analysis_model"] = model_mode
        return build_analysis_reasoning_client(resolved_config, store=self.relational_store)
    ...
    def _identity_output_root(self, series_id: str) -> Path:
        return self.artifact_storage.identity_series_root(series_id)
    ...
    def _build_booknlp_identity_bundle(
        ...
        *,
        ...
        persisted = self.identity_store.persist_identity_bundle(
            series_id=series_id,
    ...
    def _raise_if_cancelled(self, job_id: str, completed_units: int, total_units: int) -> None:
        payload = self.job_store.get_dashboard_job(job_id) or {}
    ...
    def _set_job(self, job_id: str, **updates: Any) -> dict[str, Any]:
        payload = self.job_store.get_dashboard_job(job_id) or {"id": job_id}
        payload.update(updates)
        self.job_store.upsert_dashboard_job(payload)
        return self.job_store.get_dashboard_job(job_id) or payload

    def _log(self, job_id: str, level: str, event: str, **fields: Any) -> None:
        payload = {"event": event, **fields}
        self.job_store.append_dashboard_job_log(
            job_id,
