...
from saga.providers.reasoning_runtime_adapter import MODE_CODEX, create_runtime_client
...
class WikiCharacterReferenceService:
    def __init__(
        ...
        *,
        ...
        self.llm = llm_client or create_runtime_client(mode=MODE_CODEX)
        self.web_search = web_search_tool or WebSearchService()
        self.series_id = series_id
        resolved_base = self.web_search.resolve_mediawiki_base_url(series_id=series_id, base_url=wiki_base_url)
    ...
    def _resolve_page_title(self, name: str) -> dict[str, Any]:
        ...
        search_queries = [f'title:"{name}"', str(name or "").strip()]
        search_query = search_queries[0]
        results = []
        for candidate_query in search_queries:
            search_query = candidate_query
            try:
                results = self.web_search.search_mediawiki(candidate_query, base_url=self.wiki_base_url, max_results=5).get("results") or []
            except Exception:
                results = []
            if results:
                break
        candidate_titles = [str(row.get("title") or "").strip() for row in results if str(row.get("title") or "").strip()]
    ...
    def _parse_page_html(self, page_title: str) -> str:
        payload = self.web_search.mediawiki_parse_html(page_title, base_url=self.wiki_base_url)
        return str(payload.get("html") or "")

    def _api_get(self, params: dict[str, Any]) -> dict[str, Any]:
        payload = self.web_search.mediawiki_get(params, base_url=self.wiki_base_url)
        return payload.get("payload") or {}

    def _page_url(self, page_title: str | None) -> str:
        if not page_title:
            return ""
        return self.web_search.mediawiki_page_url(page_title, base_url=self.wiki_base_url)
