...
from saga.services.image_thumbnail_service import ensure_thumbnail, thumbnail_path_for
...
def _workflow_metadata_path_for_renders(renders: list[dict[str, Any]]) -> str:
    workflow_names = {
        "character_sheet": "character_sheet_workflow.json",
        "entity_generation": "entity_generation_workflow.json",
    }
    modes = {
        str(row.get("workflow_mode") or "").strip().lower()
        for row in (renders or [])
        if str(row.get("workflow_mode") or "").strip()
    }
    if len(modes) == 1:
        filename = workflow_names.get(next(iter(modes)))
        if filename:
            return str((ROOT / "integrations" / "comfyui" / "workflows" / filename).relative_to(ROOT))
    return ""
...
class ComfyUICharacterSheetService:
    ...
    def __init__(
        ...
        *,
        ...
        self.entity_visual_prompt_service = EntityVisualPromptService(self.relational_store)
        self.comfy_pool = comfy_pool

    @property
    def sqlite_store(self) -> SagaRelationalStore:
        return self.relational_store

    @sqlite_store.setter
    def sqlite_store(self, value: SagaRelationalStore) -> None:
        self.relational_store = value
    ...
    def _active_modal_api_url(self) -> str:
        if self.comfy_pool is not None and hasattr(self.comfy_pool, "ensure_live"):
            live = self.comfy_pool.ensure_live()
            api_url = str((live or {}).get("api_url") or "").strip()
            if api_url:
                return api_url
    ...
    def _render_via_live_api(
        ...
        *,
        ...
        if self.comfy_pool is not None and hasattr(self.comfy_pool, "render"):
            payload = self.comfy_pool.render(
            ...
            self._ensure_thumbnail_best_effort(output_path)
            return
    ...
    @staticmethod
    def _ensure_thumbnail_best_effort(output_path: Path) -> None:
        try:
            ensure_thumbnail(output_path)
        except Exception:
            return
    ...
    def render_single_payload(
        ...
        *,
        ...
        render_row["thumbnail_path"] = str(thumbnail_path_for(target_path))
    ...
    def build_render_manifest(
        ...
        *,
        ...
        output_dir = self.artifact_storage.render_output_dir(contract_ref)
        images_dir = self.artifact_storage.render_images_dir(contract_ref)
        if overwrite:
        ...
        for index, row in enumerate(prompt_rows, start=1):
            ...
            resolved_entity_type = str(row.get("entity_type") or "").strip().lower()
            resolved_negative_prompt = str(row.get("negative_prompt") or "").strip()
            if not resolved_negative_prompt:
                if resolved_entity_type == "location":
                    resolved_negative_prompt = compile_location_negative_prompt()
                elif resolved_entity_type == "creature":
                    resolved_negative_prompt = compile_creature_negative_prompt()
                elif resolved_entity_type == "object":
                    resolved_negative_prompt = compile_object_negative_prompt()
                else:
        ...
        payload = {
            ...
            "workflow_path": _workflow_metadata_path_for_renders(renders),
        ...
        manifest_path = self.artifact_storage.write_render_manifest(contract_ref, payload)
        return payload
    ...
    def render_from_contract(
        ...
        *,
        ...
        manifest_path = self.artifact_storage.render_manifest_path(contract_path)
        report_path = self.artifact_storage.render_report_path(contract_path)
        self.library_store.persist_render_manifest(manifest)
        ...
        for index, row in enumerate(manifest.get("renders") or [], start=1):
            ...
            if not row.get("should_render", True) and output_path.exists():
                ...
                try:
                    ...
                    self.artifact_storage.write_render_manifest(contract_path, manifest)
                    self.artifact_storage.write_render_report(contract_path, {"renders": report_rows})
                    self.library_store.persist_render_manifest({"contract_path": manifest["contract_path"], "workflow_path": manifest["workflow_path"], "renders": [row]})
                    raise
            row["status"] = render_status
