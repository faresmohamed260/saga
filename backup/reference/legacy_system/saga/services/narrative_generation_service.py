...
class NarrativeGenerationService:
    ...
    DEFAULT_NARRATIVE_MODEL_MODE = ReasoningRuntimeClient.MODE_GPT_OSS
    ...
    def __init__(
        ...
        *,
        ...
        planner_llm_client: Optional[ReasoningClient] = None,
        prose_llm_client: Optional[ReasoningClient] = None,
        ...
        self.llm = llm_client or create_runtime_client(
            mode=self.DEFAULT_NARRATIVE_MODEL_MODE,
            model_override=self.DEFAULT_NARRATIVE_OLLAMA_MODEL,
        )
    ...
    def _prose_generation_kwargs(self, llm_client: ReasoningClient) -> Dict[str, Any]:
        mode = str(getattr(llm_client, "mode", "") or "").strip().lower()
        if mode == ReasoningRuntimeClient.MODE_GENERAL_COMPUTE:
            return {
                "temperature": 0.75,
                "max_tokens": 1400,
            }
    ...
    def _build_prose_repair_prompt(
        ...
        *,
        ...
        extra_rules = (
            "Keep the narration in close third-person only. "
            "Do not use first-person narration outside quoted dialogue. "
            "Avoid narrative lines beginning with I, we, my, our, me, us, or mine unless they are inside dialogue."
            if validation_error == "scene_prose_first_person_drift"
            else "Preserve close third-person POV and keep the prose tightly aligned to the requested scene constraints."
        )
        return (
            f"{base_prompt}\n\n"
    ...
    def _blueprint_validation_error(self, response: Any, controls: Optional[Dict[str, Any]] = None) -> str:
        ...
        if active_controls:
            requested_chapter_count = int(active_controls.get("chapter_count") or 0)
            if requested_chapter_count > 0 and int(response.get("total_chapters") or 0) != requested_chapter_count:
