...
from saga.storage.render_artifact_projection import build_render_artifact_projection


RELATIONAL_STORE = SagaRelationalStore()
...
def load_contract_like(ref: str, *, store: SagaRelationalStore | None = None) -> dict[str, Any]:
    if not is_db_book_ref(ref):
        raise ValueError(f"Unsupported relational contract ref: {ref}")
    active_store = store or RELATIONAL_STORE
    book_id = extract_book_id(ref)
    with active_store.session_factory() as session:
        book = session.get(SqlBook, book_id)
        if book is None:
            raise FileNotFoundError(f"Book not found in relational store: {ref}")
        ...
        return {
            "contract_version": "relational-adapter",
            "inputs": inputs,
            "outputs": outputs,
            "metadata": metadata,
