...
from saga.providers.reasoning_runtime_adapter import MODE_CODEX, MODE_GENERAL_COMPUTE, build_decoder_reasoning_bundle
from saga.persistence.provider_config_store import ProviderConfigStore
from saga.persistence.story_store import StoryStore
...
from saga.services.relational_contract_adapter import load_contract_like
...
class DatabaseDecoderService:
    ...
    def __init__(
        ...
        *,
        ...
        self.story_store = story_store or StoryStore(self.relational_store)
        self.provider_store = provider_store or ProviderConfigStore(self.relational_store)
    ...
    def generate_and_store(
        ...
        *,
        ...
        temp_dir = self.artifact_storage.create_decoder_run_dir(story_mode=story_mode, book_id=book_id)
        try:
        ...
        stored = self.story_store.store_generated_story(
            book_id=book_id,
            story_mode=story_mode,
    ...
    def _build_decoder(self, *, provider: str = "") -> NarrativeGenerationService:
        bundle = build_decoder_reasoning_bundle(
            provider=provider,
            store=self.relational_store,
            provider_ready=self._provider_ready,
        )
        return NarrativeGenerationService(
            llm_client=bundle["primary"],
            planner_llm_client=bundle["planner"],
            prose_llm_client=bundle["prose"],
        )

    def _provider_ready(self, provider_name: str) -> bool:
        try:
            statuses = self.provider_store.get_provider_statuses(provider_name)
        except Exception:
            return False
        for row in statuses:
    ...
    def _fallback_decoder_for_rate_limit(self, active_decoder: NarrativeGenerationService) -> NarrativeGenerationService | None:
        ...
        if planner_mode != MODE_GENERAL_COMPUTE and self._provider_ready("general_compute"):
    ...
    def _provider_key_for_decoder(self, decoder: NarrativeGenerationService) -> str:
        ...
        if planner_mode == MODE_GENERAL_COMPUTE and llm_mode == MODE_GENERAL_COMPUTE:
            return "general_compute"
        if llm_mode == MODE_CODEX:
            return "codex"
        return "ollama"
