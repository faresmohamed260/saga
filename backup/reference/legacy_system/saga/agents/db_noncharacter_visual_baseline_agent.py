...
from saga.domain.trait_taxonomy import practical_persistent_fields
...
"""
    ...
    def _retrieve_semantic_evidence(self, *, bundle: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        rows = self._query_book_retrieval(
    ...
    def _clean_location_text(self, value: Any, *, allow_sentence: bool = False) -> str:
        ...
        if not allow_sentence:
            if len(text.split()) <= 4 and " and " in lowered and not any(mark in text for mark in ",.;"):
                return text
