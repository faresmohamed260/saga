"""Workspace-scoped Modal URL resolution helpers for xcore-litbank deployments."""
...
try:
    from integrations.xcore_litbank.token_pool import ModalToken
...
import json
import modal
cls = modal.Cls.from_name({app_name!r}, "XCoreLitbankService")
...
def deploy_app(token: ModalToken) -> str:
    result = _run([str(MODAL_EXE), "deploy", str(MODAL_APP_FILE)], token=token, timeout=3600)
    if result.returncode != 0:
