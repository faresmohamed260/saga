...
```powershell
venv\Scripts\python.exe integrations\comfyui\run_gateway.py
```
...
```powershell
venv\Scripts\python.exe integrations\comfyui\gateway_service.py --startup auto install
venv\Scripts\python.exe integrations\comfyui\gateway_service.py start
```

To remove it later:

```powershell
venv\Scripts\python.exe integrations\comfyui\gateway_service.py stop
venv\Scripts\python.exe integrations\comfyui\gateway_service.py remove
```
...
```powershell
powershell -ExecutionPolicy Bypass -File integrations\comfyui\install_gateway_task.ps1
```

Remove it with:

```powershell
powershell -ExecutionPolicy Bypass -File integrations\comfyui\uninstall_gateway_task.ps1
```

If Scheduled Tasks are also blocked by policy on this PC, use the Startup-folder fallback:

```powershell
powershell -ExecutionPolicy Bypass -File integrations\comfyui\install_gateway_startup.ps1
```
