...
LOCATION_WORKFLOW_PATH = "/root/location_workflow.json"
...
LOCAL_LOCATION_WORKFLOW = Path(__file__).parent / "workflows" / "location_workflow.json"
...
class ComfyService:
    ...
    def _apply_default_inputs(
        ...
        *,
        ...
        _set_if_present(workflow, "7", "text", negative_prompt)
        _set_if_present(workflow, "3", "seed", seed)
        _set_if_present(workflow, "3", "steps", steps)
        _set_if_present(workflow, "3", "cfg", cfg)
        _set_if_present(workflow, "5", "width", width)
        _set_if_present(workflow, "5", "height", height)
        _set_if_present(workflow, "9", "filename_prefix", filename_prefix)
        return workflow

    def _apply_location_inputs(
        self,
        workflow: dict[str, Any],
        *,
        prompt: str,
...
def entrypoint(
    prompt: str = "",
    output_path: str = "integrations/comfyui/outputs/render.png",
    output_dir: str = "",
    manifest_path: str = "",
    report_path: str = "",
    negative_prompt: str = "blurry, low quality, distorted hands, artifacts",
    ...
    if manifest_path:
        ...
        output_root = Path(output_dir or manifest.get("images_dir") or "integrations/comfyui/outputs")
