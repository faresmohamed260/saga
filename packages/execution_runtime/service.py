...
class PersistenceTelemetryExporter:
    ...
    def export(self, *, run_id: str, queue_item: dict[str, Any], events: list[dict[str, Any]]) -> dict[str, Any]:
        request = dict(queue_item.get("payload") or {}).get("orchestration_request") or {}
        return self.persistence.artifacts.store_json(
            artifact_type="runtime_report",
            filename=f"{run_id}-execution-telemetry.json",
            payload={"run_id": run_id, "queue_item": _sanitized_queue_item(queue_item), "events": events},
            series_id=str(request.get("series_id") or ""), story_id=str(request.get("story_id") or ""), run_id=run_id,
            provider_name="execution_runtime", report_kind="telemetry",
            metadata={"queue_name": queue_item.get("queue_name"), "terminal_status": queue_item.get("status")},
        )


class PersistenceExecutionObserver:
    def __init__(self, persistence, *, retention_days: int = 30, otlp_http_endpoint: str = "") -> None:
        self.persistence = persistence
        exporters = [OTLPHTTPExporter(otlp_http_endpoint)] if str(otlp_http_endpoint or "").strip() else []
    ...
    def observe(self, *, run_id: str, queue_item: dict[str, Any], events: list[dict[str, Any]], orchestration_result) -> dict[str, Any]:
        request = dict(queue_item.get("payload") or {}).get("orchestration_request") or {}
        ...
        for stage in {str(row.get("stage") or "") for row in current_lineage}:
            lineage.extend(self.persistence.lineage.list(series_id=series_id, stage=stage, limit=1000))
        active_statuses = {"queued", "retry_wait", "leased", "cancel_requested"}
        queue_depth = sum(1 for item in self.persistence.execution_queue.list(queue_name=queue_item.get("queue_name"), limit=10000) if item.get("status") in active_statuses)
        observed = self.runtime.observe_execution(
            ...
            trace_payloads=load_lineage_trace_snapshots(self.persistence, current_lineage),
...
class ExecutionRuntimeService:
    def __init__(self, *, config: ExecutionRuntimeServiceConfig) -> None:
        ...
        profile = PersistenceProfile(
            name="execution-runtime", provider=config.persistence_provider, mode=config.persistence_mode,
            database_url=config.database_url, application_name="saga-execution-runtime",
        ...
        self.persistence = create_persistence_client(
            ...
            config=PersistenceRuntimeConfig(
                profile=profile, supabase_api_url=config.supabase_api_url,
                supabase_anon_key=config.supabase_anon_key, supabase_service_role_key=config.supabase_service_role_key,
            ),
        )
        self.persistence.initialize()
        self.queue = ExecutionQueueRuntime(persistence=self.persistence, queue_name=config.queue_name)
        self.queue.configure(ExecutionQueuePolicy(
            global_limit=config.global_limit, per_series_limit=config.per_series_limit,
            default_capability_limit=config.default_capability_limit, capability_limits=config.capability_limits,
        ))
        self.orchestration_config = ProductionOrchestrationServiceConfig(
            persistence_mode=config.persistence_mode, persistence_provider=config.persistence_provider,
            database_url=config.database_url, local_storage_root_dir=config.local_storage_root_dir,
    ...
    def submit(self, request: OrchestrationRequest, *, priority: int = 0, max_attempts: int = 3, backoff_seconds: int = 10) -> dict[str, Any]:
        return self.queue.submit(ExecutionSubmission(
            queue_id=queue_id_for_run(request.run_id), request=request, priority=priority,
    ...
    def retry(self, request: OrchestrationRequest, *, priority: int = 0, max_attempts: int = 3) -> dict[str, Any] | None:
        return self.queue.requeue(ExecutionSubmission(
            queue_id=queue_id_for_run(request.run_id), request=request, priority=priority, max_attempts=max_attempts,
        ))

    def run_worker_once(self, *, worker_id: str = "") -> WorkerExecutionResult:
        resolved_worker = worker_id or f"{socket.gethostname()}-{os.getpid()}-{uuid.uuid4().hex[:6]}"
        worker = ExecutionWorker(
            queue=self.queue,
            executor_factory=lambda checker: ProductionOrchestrationService(config=self.orchestration_config, cancellation_checker=checker),
            telemetry_exporter=PersistenceTelemetryExporter(self.persistence),
            observer=PersistenceExecutionObserver(self.persistence, retention_days=self.config.observability_retention_days, otlp_http_endpoint=self.config.otlp_http_endpoint),
...
def load_execution_runtime_service_config_from_env() -> ExecutionRuntimeServiceConfig:
    ...
    limits = json.loads(limits_raw) if limits_raw else ExecutionRuntimeServiceConfig().capability_limits
    ...
    return ExecutionRuntimeServiceConfig(
        ...
        queue_name=str(os.getenv("SAGA_EXECUTION_QUEUE_NAME") or "production-orchestration").strip(),
        ...
        per_series_limit=max(1, int(os.getenv("SAGA_EXECUTION_PER_SERIES_LIMIT") or "1")),
        default_capability_limit=max(1, int(os.getenv("SAGA_EXECUTION_DEFAULT_CAPABILITY_LIMIT") or "2")),
        observability_retention_days=max(1, int(os.getenv("SAGA_OBSERVABILITY_RETENTION_DAYS") or "30")),
...
def load_lineage_trace_snapshots(persistence, records: list[dict[str, Any]], *, max_snapshots: int = 100, max_characters: int = 10_000_000) -> list[dict[str, Any]]:
    snapshots: list[dict[str, Any]] = []
    for row in records[-max(1, max_snapshots):]:
        reference = dict(row.get("payload") or {}).get("output_artifact_version") or {}
        bucket, path = str(reference.get("bucket_name") or ""), str(reference.get("object_path") or "")
...
def default_execution_slos() -> list[SLODefinition]:
    return [
        SLODefinition(slo_id="run-success-rate", metric_name="run.success", comparator="gte", threshold=0.95, aggregation="average", minimum_samples=5, severity="critical"),
        SLODefinition(slo_id="queue-wait-p95", metric_name="queue.wait", comparator="lte", threshold=60.0, aggregation="p95", minimum_samples=5),
        SLODefinition(slo_id="dead-letter-free", metric_name="queue.dead_letter", comparator="lte", threshold=0.0, aggregation="sum", minimum_samples=1, severity="critical"),
        SLODefinition(slo_id="lease-expiry-free", metric_name="queue.lease_expiry", comparator="lte", threshold=0.0, aggregation="sum", minimum_samples=1),
        SLODefinition(slo_id="stage-acceptance", metric_name="stage.accepted", comparator="gte", threshold=0.95, aggregation="average", minimum_samples=5, severity="critical"),
        SLODefinition(slo_id="provider-error-rate", metric_name="provider.error", comparator="lte", threshold=0.05, aggregation="average", minimum_samples=5, severity="critical"),
    ]
