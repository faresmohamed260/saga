...
async function main() {
  ...
  if (!normalizationPath || !identityPath) {
    throw new Error(
      "usage: dialogue-deterministic-cli --normalization <normalization.json> --identity <identity-result.json> [--out <prediction.json>]",
    );
  }
  ...
  const prediction = predictDeterministicDialogue({
