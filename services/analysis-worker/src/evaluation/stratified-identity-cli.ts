...
import {
  ...
  type NovelDiversityManifest,
  type StratifiedIdentitySourceReport,
} from "./stratified-identity-benchmark.js";
...
function parseArgs(argv: string[]) {
  ...
  if (!report || !manifest || !out) {
    throw new Error(
      "usage: stratified-identity-cli --report <provider-report.json> --manifest <diversity-manifest.json> --out <report.json>",
    );
  }
...
async function main() {
  ...
  const output = buildStratifiedIdentityBenchmark({
    report: JSON.parse(rawReport) as StratifiedIdentitySourceReport,
