...
function parseArgs(argv: string[]) {
  ...
  if (!litbankRoot || !booknlpRoot || !out) {
    throw new Error("usage: event-dependency-litbank-cli --litbank-root <root> --booknlp-root <outputs> --out <report.json> [--limit N]");
  }
...
async function main() {
  ...
  const semantic = {
    schemaVersion: "saga-event-dependency-litbank-diagnostic-v1",
    dataset: {
