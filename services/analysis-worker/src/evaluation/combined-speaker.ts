...
export const COMBINED_SPEAKER_VERSION = "saga-dialogue-combined-speaker-v2";

export type CombinedSpeakerConflictPolicy =
  | "unresolved"
  | "prefer_provider_after_speech_tag";

export const DEFAULT_COMBINED_SPEAKER_CONFLICT_POLICY: CombinedSpeakerConflictPolicy =
  "prefer_provider_after_speech_tag";
...
function isPostQuoteSpeechTag(reason: string) {
  return reason.startsWith("speech_verb_linked_mention:after:");
}
...
function fuseSpeaker(input: {
  ...
  conflictPolicy: CombinedSpeakerConflictPolicy;
  ...
  if (deterministicSpeakerKey && input.providerSpeakerKey) {
    ...
    if (
      input.conflictPolicy === "prefer_provider_after_speech_tag"
      && isPostQuoteSpeechTag(input.deterministic.decisionReason)
    ) {
      return {
        speakerKey: input.providerSpeakerKey,
        decisionReason: `combined_conflict_provider_after:${input.deterministic.decisionReason}:${input.providerReason}`,
      };
    }
    return {
      speakerKey: null,
...
export function predictCombinedSpeakerDialogue(input: {
  ...
  const conflictPolicy = input.conflictPolicy ?? DEFAULT_COMBINED_SPEAKER_CONFLICT_POLICY;
