...
const tokensTsv = `paragraph_ID\tsentence_ID\ttoken_ID_within_sentence\ttoken_ID_within_document\tword\tlemma\tbyte_onset\tbyte_offset\tPOS_tag\tfine_POS_tag\tdependency_relation\tsyntactic_head_ID\tevent
...
const entitiesTsv = `COREF\tstart_token\tend_token\tprop\tcat\ttext
1\t0\t0\tPROP\tPER\tÉlodie
2\t4\t4\tPROP\tPER\tAlice
3\t10\t10\tPROP\tPER\tBob
3\t13\t13\tPROP\tPER\tBob`;

const quotesTsv = `quote_start\tquote_end\tmention_start\tmention_end\tmention_phrase\tchar_id\tquote
...
test("BookNLP output becomes source-anchored identity, quote, and event evidence", () => {
  ...
  const result = resolveCharacterIdentity(evidence.identityEvidence, text);
