...
export function ShellHeader({ state, loading, latestLabel, latestDetail, onRefresh }) {
  return (
    <header className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/70 shadow-2xl shadow-black/20">
      <div className="grid gap-0 lg:grid-cols-[1fr_340px]">
        <div className="px-5 py-6 md:px-7 md:py-7">
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-white md:text-4xl">Operations Console</h1>
          ...
          <p className="mt-4 max-w-full truncate text-sm text-slate-500">{state?.workspace?.root || "Loading project root..."}</p>
        </div>
