...
alter table public.saga_characters
  add column resolver_key text;

alter table public.saga_characters
  add constraint saga_characters_resolver_key_check
  check (resolver_key is null or char_length(btrim(resolver_key)) between 1 and 160);

create unique index saga_characters_run_resolver_key_unique
  on public.saga_characters (run_id, resolver_key)
  where resolver_key is not null;

alter table public.saga_character_mentions
  add column provider_evidence_id text;

alter table public.saga_character_mentions
  add constraint saga_character_mentions_provider_evidence_id_check
  check (
    provider_evidence_id is null
    or char_length(btrim(provider_evidence_id)) between 1 and 256
  );

create unique index saga_character_mentions_run_provider_evidence_unique
  on public.saga_character_mentions (run_id, provider_evidence_id)
  where provider_evidence_id is not null;

create or replace function public.saga_enqueue_character_identity_after_ingestion()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.kind <> 'source_ingestion'
     or new.status <> 'succeeded'
     or old.status = 'succeeded' then
    return new;
  end if;

  select run.output_fingerprint
    into v_input_fingerprint
  from public.saga_analysis_runs as run
  where run.job_id = new.id
  ...
  if v_input_fingerprint is null then
    raise exception 'saga_missing_ingestion_run_for_identity_handoff';
  end if;

  if not exists (
    select 1
    from public.saga_analysis_jobs as existing
    where existing.source_id = new.source_id
      and existing.kind = 'character_identity'
      and existing.input_fingerprint = v_input_fingerprint
...
revoke all on function public.saga_enqueue_character_identity_after_ingestion()
  from public, anon, authenticated;

create trigger saga_analysis_jobs_enqueue_identity_after_ingestion
  after update of status on public.saga_analysis_jobs
  for each row
  when (old.status is distinct from new.status)
  execute function public.saga_enqueue_character_identity_after_ingestion();
...
begin
  ...
  if exists (
    ...
    select 1
    from (
      select evidence_id, count(*) as row_count
      from jsonb_to_recordset(p_mentions) as mention(evidence_id text)
...
create or replace function public.saga_service_commit_identity_failure(
  p_job_id uuid,
  p_lease_token uuid,
  p_resolver_version text,
  p_provider_name text,
...
begin
  ...
  if p_resolver_version is null
    ...
    raise exception 'saga_invalid_identity_failure';
  end if;

  select normalized.normalized_sha256
...
revoke all on function public.saga_service_commit_identity_failure(uuid,uuid,text,text,text,text,text,text,text)
  from public, anon, authenticated;
grant execute on function public.saga_service_commit_identity_failure(uuid,uuid,text,text,text,text,text,text,text)
  to service_role;
