...
BEGIN
  ...
  FROM public.saga_analysis_jobs
  WHERE id = current_setting('identity_claim_job_id', true)::uuid;
  ...
  WHERE id = current_setting('identity_run_id', true)::uuid;

  SELECT count(*) INTO v_character_count
  FROM public.saga_characters
  WHERE run_id = current_setting('identity_run_id', true)::uuid
