...
expect(controls.includes("title={videoAudio ? 'Audio enabled' : 'Audio disabled'}"), 'Audio control must expose state tooltip copy');
...
expect(css.includes('content: attr(title);'), 'Audio state tooltip must expose the title copy visually');
...
console.log('Audio control contract passed: explicit On/Off text, tooltip copy, aria-pressed state, focus treatment, and compact mobile behavior are wired.');
