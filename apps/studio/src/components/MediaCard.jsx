...
export default function MediaCard({
  ...
  const morePopover = moreOpen ? (
    <div ref={morePopoverRef} className="media-actions-popover" role="menu" aria-label="More media actions" onClick={(event) => event.stopPropagation()}>
      ...
      {item.kind !== 'video' && <button type="button" role="menuitem" onClick={menuAction(onAnimate)}><Video size={15}/><span>Animate</span></button>}
  ...
  const standardActions = (
    <div className="card-actions">
      ...
      {item.kind !== 'video' && <button type="button" title="Animate this" aria-label="Animate this" onClick={action(onAnimate)}><Video size={16}/></button>}
