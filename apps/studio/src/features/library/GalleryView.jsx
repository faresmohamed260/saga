    ...
    <section className={`gallery-view gallery-density-${density}`}>
      ...
      <div className="gallery-library-nav" aria-label="Asset library navigation">
        <div className="gallery-primary-tabs" role="tablist" aria-label="Asset libraries">
          <button type="button" role="tab" aria-selected="true" className="selected"><Sparkles size={16}/><span>Creatives</span></button>
          <button type="button" role="tab" aria-selected="false" aria-disabled="true" disabled title="Uploads library is not available yet"><Upload size={16}/><span>Uploads</span></button>
        ...
        <label className="gallery-search gallery-search-desktop">
          ...
          <input type="search" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search" aria-label="Search prompts" />
        </label>
        <button type="button" className={`gallery-mobile-search-trigger ${mobileSearchOpen ? 'active' : ''}`} aria-label="Search Gallery" aria-pressed={mobileSearchOpen} onClick={() => setMobileSearchOpen((value) => !value)}><Search size={20}/></button>
      </div>
      ...
      {mobileSearchOpen && (
        <label className="gallery-search gallery-search-mobile">
          <Search size={16}/>
          <span className="sr-only">Search prompts</span>
          <input autoFocus type="search" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="Search prompts" aria-label="Search prompts" />
        </label>
      )}
